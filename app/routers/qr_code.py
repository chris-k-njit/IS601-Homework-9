...
# @router.post("/qr-codes/", response_model=QRCodeResponse, status_code=status.HTTP_200_OK, tags=["QR Codes"]) # qr-coes adjusted to qr-codes
@router.post("/qr-codes/", response_model=QRCodeResponse, status_code=status.HTTP_201_CREATED, tags=["QR Codes"]) # adjusted to 201 created, as indicated in comment above and while reviewing lesson videos.
async def create_qr_code(request: QRCodeRequest, token: str = Depends(oauth2_scheme)):
    ...
    if qr_code_full_path.exists():
        ...
        return JSONResponse(
            # status_code=status.HTTP_200_OK, incorrect here, adjusted after rewatching lecture video
...
# @router.delete("/qr-codes/{qr_fileame}", status_code=status.HTTP_200_OK, tags=["QR Codes"]) -- as noted in lecture video and materials, this is incorrect
